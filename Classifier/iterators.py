...
class ParallelBatchIterator(object):
    ...
    def __init__(self, X, batch_size, dataset, shuffle=False, preprocess=False):
    ...
    def process_temp(self, key_x, path='aurora2/'):
    ...
    def gen(self, indices, temp=True):
        ...
        for i in range(len(key_batch_x)):
            ...
            if temp:
                X = self.process_temp(key_batch_x[i])
                X_batch_new[i] = X

        return X_batch_new, key_batch_x
