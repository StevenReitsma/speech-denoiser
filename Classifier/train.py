...
if __name__ == "__main__":
    ...
    #only select clean files
    X_complete = list(X_train)
    X_train = [s for s in X_complete if s[:5] == 'clean']
    X_test = [s for s in X_complete if s[:5] == 'multi']


    print('Data size train: ', len(X_train))
    print('Data size test: ', len(X_test))
    # 1/3 validation
    ...
    net = NeuralNetwork(params.BATCH_SIZE, X_train, X_valid, X_test)
    net.fit()
